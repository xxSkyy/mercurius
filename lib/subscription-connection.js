...
module.exports = class SubscriptionConnection {
    ...
    switch (type) {
      ...
      case GQL_START: {
        ...
        if (this.isReady) {
          ...
          this.sendMessage(GQL_CONNECTION_ERROR, undefined, {
  ...
  async handleConnectionInit(data) {
    ...
    if (data.payload.headers.authorization) {
      try {
  ...
  async handleWSqueries(data) {
    ...
    return // These complete are useless when data or error are always sent?
