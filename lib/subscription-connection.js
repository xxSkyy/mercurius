...
const gql = require("graphql-tag") // Need to have
const { getMainDefinition } = require("@apollo/client/utilities") // Need to have
const jwt_decode = require("jwt-decode") // Need to have
const { subscribe, parse, graphql, print, getOperationAST  } = require('graphql')
...
module.exports = class SubscriptionConnection {
      ...
      case this.protocolMessageTypes.GQL_START: {
        ...
        try {
          if (typeof data.payload.query === "string") {
            const query = gql`
              ${data.payload.query.toString()}
            `
            const { operation } = await getMainDefinition(query)
            queryType = operation
          }
        } catch {}
        ...
        if (this.isReady) {
          if (queryType === "query" || queryType === "mutation") {
            this.handleWSqueries(data).catch((e) => {
              this.sendMessage(this.protocolMessageTypes.GQL_CONNECTION_ERROR, id, e.message)
            })
          } else {
          ...
          this.sendMessage(this.protocolMessageTypes.GQL_CONNECTION_ERROR, undefined, {
  ...
  async handleConnectionInit(data) {
    ...
    try {
      const token = data.payload.headers.authorization
      const decoded = jwt_decode(token)
      this.context.user = decoded
    } catch {}
  ...
  async handleWSqueries(data) {
    ...
    try {
      ...
      this.sendMessage(this.protocolMessageTypes.GQL_DATA, data.id, response)
    } catch (e) {
      this.sendMessage(this.protocolMessageTypes.GQL_ERROR, null, e)
    }
    ...
    if (extensions && extensions instanceof Array) {
      for (const extension of extensions) {
        switch (extension.type) {
          case "connectionInit": {
            const authorize = await this.handleConnectionInitExtension(
              extension
            )
            if (typeof authorize === "object") {
              context = {
  ...
  async _executeSubscription ({ document, query, context, variables, operationName, id }) {
    ...
    if (
      this.context.preSubscriptionParsing !== null &&
    ...
    if (
      this.context.preSubscriptionExecution !== null &&
      typeof schema !== "undefined"
    ) {
  ...
  handleConnectionClose() {
    Array.from(this.subscriptionContexts.values()).map((sc) => sc.close())
    Array.from(this.subscriptionIters.values()).map(
      (subIter) => subIter.return && subIter.return()
    )
    this.socket.close()
